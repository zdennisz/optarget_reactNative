...
const FieldOfView = (props) => {
	...
	const calcHandler = () => {
		if (focalLength && detPitch && det_w && det_h) {
			const hFovVal = calcHfov(
				detectorData.detectorPitch,
				settingsData.detectorSize.width,
				detectorData.focalLength
			);

			const vFovVal = calcVfov(
				settingsData.detectorSize.height,
				settingsData.detectorSize.width,
				hFovVal
			);

			const ifovval = calcIfov(
				detectorData.detectorPitch,
				detectorData.focalLength
			);
	...
	const calcHfov = (sp, szW, fl) => {
		return (2 * Math.atan((sp * szW) / (2000 * fl)) * (2 * 90)) / Math.PI;
	};
				...
				<View style={styles.outputContainer}>
					{isValid && (
						<View style={styles.outputCom}>
							<OutputTextView
								text='HFOV (deg):'
								image={FieldOfViewImages.hfovSrc}
								result={result.hfov}
								resLength={1}
							/>
							<OutputTextView
								text='VFOV (deg):'
								image={FieldOfViewImages.vfovSrc}
								result={result.vfov}
								resLength={1}
							/>
							<OutputTextView
